...
export const getDocuments = async (req: Request, res: Response) => {
  try {
    const response = await axios.get(
      `${URL}result/query?type=published&query=showindocumentrepository%3Dtrue&offset=0&limit=-1&fields=audience%2Ccategory%2Cclassification%2Cdocumentstatus%2Cexternal%2Cfunctionsubfn%2Cname%2Cpublishdate%2Creleasedate%2Creviewdate%2Ctype%2Cdownloadoriginalfiletype%2Cversion%2Cversionstatus%2Crelated&header=false&datameta=false`,
      {
        headers: {
          Authorization: auth,
        },
